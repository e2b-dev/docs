...
async def main():
    ...
    # You can now connect the gateway to any MCP client, for example claude:
    await sbx.commands.run(f'claude mcp add --transport http e2b-mcp-gateway {mcp_url} --header "Authorization: Bearer {mcp_token}"', timeout=50, on_stdout=print, on_stderr=print)
