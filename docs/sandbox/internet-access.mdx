...
# Request without token will fail with 403
# Request with token header 'e2b-traffic-access-token' will succeed
```
</CodeGroup>
...
// Requests to the sandbox will have Host header set to: custom-host.example.com:8080
```
```python Python
from e2b_code_interpreter import Sandbox
...
sandbox = Sandbox.create(
    network={
        "mask_request_host": "custom-host.example.com:${PORT}"
    }
)
...
The `${PORT}` variable in the mask will be automatically replaced with the actual port number of the service. By default, the host format is `${PORT}-sandboxid.e2b.app`.
