...
Sandboxes are now **secure by default**. That means you cannot access the sandbox controller directly via its URL; there's authentication required.
If you use the SDK, this is enabled for you automatically.

<Note>
    If you are using older custom templates, you may receive error message that secure communication is not yet supported.
    Template rebuild will fix that.
</Note>

If you are accessing sandbox controller URLs directly, you need to provide an authentication token that was returned during sandbox creation,
or disable secure communication explicitly by setting `secure` to `false` during sandbox creation.

<Warning>
    Setting `secure` to `false` disables secure communication and anyone with sandbox id can control your sandbox, so use with caution.
</Warning>
