...
If you need to use a [custom sandbox template](/docs/legacy/guide/custom-sandbox) you pass the template as the first parameter:
<CodeGroup isRunnable={false}>
```js JavaScript & TypeScript
import Sandbox from 'e2b'
...
<Note>
Check of the [Code Interpreter custom template](/docs/legacy/code-interpreter/template) page for more info.
</Note>
